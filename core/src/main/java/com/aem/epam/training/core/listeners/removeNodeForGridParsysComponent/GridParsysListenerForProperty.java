...
public class GridParsysListenerForProperty implements EventListener {
    ...
    public void activate(ComponentContext context) {
        ...
        try {
            ...
            session.getWorkspace().getObservationManager().addEventListener(
                    this, //handler
                    Event.PROPERTY_CHANGED,  //binary combination of event types
        ...
        try {
            while (eventIterator.hasNext()) {
                ...
                if (checkEventPath(eventPath)) {
                    String pathToGridParNode = PathUtils.getAncestorPath(eventPath, 1);
                    final Map<String, Object> props = new HashMap<String, Object>();
                    props.put("pathToGridParNode", pathToGridParNode);

                    jobManager.addJob(JOB_TOPIC_CHANGE_PROPERTY, props);
                } else {
                    continue;
                }
    ...
    private boolean checkEventPath(String eventPath) {
        return eventPath.contains("countRow") || eventPath.contains("countColumns");
    }
}
